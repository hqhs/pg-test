...
var (
	dsnPgbouncer1120Double = fmt.Sprintf("host=%s port=6435 database=db1 user=postgres sslmode=disable", dockerHost)
	dsnPgbouncer1901120    = fmt.Sprintf("host=%s port=6433 database=db1 user=postgres sslmode=disable", dockerHost)

	dsnPgbouncer1120 = fmt.Sprintf("host=%s port=6434 database=db1 user=postgres sslmode=disable", dockerHost)
	dsnPgbouncer172  = fmt.Sprintf("host=%s port=6432 database=db1 user=postgres sslmode=disable", dockerHost)
)

func TestCancelRequest(t *testing.T) {
	t.Run("double pgbouncer 1.12.0", func(t *testing.T) {
		db, err := sql.Open("postgres", dsnPgbouncer1120Double)
		require.Nil(t, err)

		testCancelRequest(t, db)
	})

	t.Run("pgbouncer 1.9.0 -> 1.12.0", func(t *testing.T) {
		db, err := sql.Open("postgres", dsnPgbouncer1120)
		require.Nil(t, err)

		testCancelRequest(t, db)
	})

	t.Run("pgbouncer 1.12.0", func(t *testing.T) {
		db, err := sql.Open("postgres", dsnPgbouncer1120)
		require.Nil(t, err)
